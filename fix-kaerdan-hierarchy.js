...
async function fixKaerdanHierarchy() {
  try {
    console.log('🏰 Fixing Kaerdan Confederation hierarchy...');
    
    // Find the Kaerdan region
    const region = await prisma.location.findFirst({
      where: {
        name: { contains: "Kaerdan" },
        tier: 'REGION'
      }
    });
    
    if (!region) {
      console.log('❌ Kaerdan region not found');
      return;
    }
    ...
    const cityAssignments = {
      'Goldport': ['Arena dei Campioni', 'Crossroads Inn'],
      'Dhorvar': ['Tomba del Re Perduto', 'Ponte del Giuramento'],
      'Ironhold': ['Hilltown'],
      'Farmlands': ['Woodhaven']
    };
    ...
    console.log('\n✅ Kaerdan Confederation hierarchy fixed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing Kaerdan hierarchy:', error);
  } finally {
