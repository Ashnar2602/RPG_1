...
async function fixElfDomainHierarchy() {
  try {
    console.log('🌿 Fixing Elf Domain hierarchy...');
    
    // Find the Elf Domain region  
    const region = await prisma.location.findFirst({
      where: {
        name: { contains: "Altherys" },
        tier: 'REGION'
      }
    });
    
    if (!region) {
      console.log('❌ Elf Domain region not found');
      return;
    }
    ...
    const cityAssignments = {
      'Whisperwind': ['Greenwood Market', 'Crystalbrook Crossing'],
      'Elarion': ['Starfall Grove'],
      'Starfall Citadel': ['Silverleaf', 'Windsong'],
      'Silvermoon': ['Moonwell Village']
    };
    ...
    console.log('\n✅ Elf Domain hierarchy fixed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing Elf Domain hierarchy:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fixElfDomainHierarchy();
