...
async function fixTrollTerritoriesHierarchy() {
  try {
    console.log('❄️ Fixing Troll Territories hierarchy...');
    
    // Find the Troll Territories region  
    ...
    if (!region) {
      console.log('❌ Troll Territories region not found');
      return;
    }
    ...
    const cityAssignments = {
      'Bonechill Caves': ['Crystalfrost Mine', 'Frostbite Camp'],
      'Glacier\'s Heart': ['Permafrost Storage', 'Icicle Village'],
      'Frosthold Keep': ['Blizzard\'s Rest'],
      'Stormwall Citadel': ['Snowdrift Outpost']
    };
    ...
    console.log('\n✅ Troll Territories hierarchy fixed successfully!');
    
  } catch (error) {
    console.error('❌ Error fixing Troll Territories hierarchy:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fixTrollTerritoriesHierarchy();
